...
struct jetEECTask {
  ...
  void init(o2::framework::InitContext&)
  {
    ...
    if (doprocessJetsData || doprocessJetsMCD || doprocessJetsMCDWeighted) {
      registry.add("h_jet_deltaRL", "jet pT;#it{p}_{T,jet} (GeV/#it{c});entries", {HistType::kTH1F, {jetPtAxis}});
  ...
  PROCESS_SWITCH(jetEECTask, processJetsData, "jet finder QA data", false);
  ...
  void processJetsMCD(soa::Filtered<aod::JetCollisions>::iterator const& collision, soa::Join<aod::ChargedMCDetectorLevelJets, aod::ChargedMCDetectorLevelJetConstituents> const& jets, aod::JetTracks const&)
  {
    ...
    for (auto const& jet : jets) {
      ...
      const auto& constituents = jet.template.tracks_as<aod::JetTracks>();
      for (int i = 0; i < constituents.size(); i++) {
        for (int j = i+1; i < constituents.size(); j++) {
          const auto& constituent_i = constituents.iteratorAt(i);
          const auto& constituent_j = constituents.iteratorAt(j);

          float deltaRL = jetutilities::deltaR(constituent_i, constituent_j);

          float weight = (constituent_i.pt() * constituent_j.pt()) / (jet.pt()*jet.pt());
          //weight = weight * jet.eventWeight();
          registry.fill(HIST("h_jet_deltaRL"), deltaRL, weight);
        }
      }
    }
  }
  PROCESS_SWITCH(jetEECTask, processJetsMCD, "jet finder QA mcd", false);
...
/////
  ...
  PROCESS_SWITCH(jetEECTask, processJetsMCDWeighted, "jet finder QA mcd with weighted events", false);

};

WorkflowSpec defineDataProcessing(ConfigContext const& cfgc) { return WorkflowSpec{adaptAnalysisTask<jetEECTask>(cfgc, TaskName{"jet-finder-charged-qa"})}; }
