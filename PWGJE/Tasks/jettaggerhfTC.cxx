...
/// \file jettaggerhfTC.cxx
/// \brief Jet tagging using track counting method
///
...
struct JetTaggerHFTC {
  ...
  ConfigurableAxis binImpactParameterXY{"binImpactParameterXY", {200, -0.04f, 0.04f}, ""};
  //ConfigurableAxis binImpactParameterSignificanceXY{"binImpactParameterSignificanceXY", {400, -0.004f, 0.004f}, ""}; // true
  ConfigurableAxis binImpactParameterSignificanceXY{"binImpactParameterSignificanceXY", {200, -40.f, 40.f}, ""}; // test
  ConfigurableAxis binImpactParameterXYZ{"binImpactParameterXYZ", {500, -0.1f, 0.1f}, ""};
  ConfigurableAxis binImpactParameterSignificanceXYZ{"binImpactParameterSignificanceXYZ", {500, -100.f, 100.f}, ""};
  ...
  AxisSpec ImpactParameterSignificanceXYAxis = {binImpactParameterSignificanceXY, "IPs_{XY}"};
  AxisSpec ImpactParameterXYZAxis = {binImpactParameterXYZ, "IP_{XYZ} [cm]"};
  AxisSpec ImpactParameterSignificanceXYZAxis = {binImpactParameterSignificanceXYZ, "IPs_{XYZ}"};
  ...
  void init(InitContext const&)
  {
    for (int i = 0; i < 4; i++) {
      ...
      /// TEMP ///
      registry.add(Form("h_%s_distance_x_track_pv_global", charJetFlavor[i].data()), Form("%s sign x of distance between track and PV in global", charJetFlavor[i].data()), {HistType::kTH1F, {ImpactParameterXYAxis}});
      registry.add(Form("h_%s_distance_x_track_pv_local", charJetFlavor[i].data()), Form("%s sign x of distance between track and PV in local", charJetFlavor[i].data()), {HistType::kTH1F, {ImpactParameterXYAxis}});
      registry.add(Form("h_%s_dca_x_track_pv", charJetFlavor[i].data()), Form("%s dca_{x}", charJetFlavor[i].data()), {HistType::kTH1F, {ImpactParameterXYAxis}});
      registry.add(Form("h_%s_dca_y_track_pv", charJetFlavor[i].data()), Form("%s dca_{y}", charJetFlavor[i].data()), {HistType::kTH1F, {ImpactParameterXYAxis}});
      registry.add(Form("h_%s_abs_dca_xy_track_pv", charJetFlavor[i].data()), Form("%s abs dca_{x}", charJetFlavor[i].data()), {HistType::kTH1F, {ImpactParameterXYAxis}});
      registry.add(Form("h_%s_abs_impact_parameter_xy", charJetFlavor[i].data()), Form("%s abs impact parameter dca_{xy}", charJetFlavor[i].data()), {HistType::kTH1F, {ImpactParameterXYAxis}});
      // compare between track and track iu for dca
      registry.add(Form("h_%s_distance_x_trackiu_pv_global", charJetFlavor[i].data()), Form("%s sign x of distance between trackIU and PV in global", charJetFlavor[i].data()), {HistType::kTH1F, {ImpactParameterXYAxis}});
      registry.add(Form("h_%s_dca_x_trackiu_pv", charJetFlavor[i].data()), Form("%s trackIU dca_{x}", charJetFlavor[i].data()), {HistType::kTH1F, {ImpactParameterXYAxis}});
      ////////////
  ...
  using JetTagTracks = soa::Join<aod::JTracks, aod::JTrackPIs, aod::JMcTrackLbs>;
  //using OriTracks = soa::Join<aod::Tracks, aod::TracksExtra, aod::TracksDCA, aod::TracksDCACov, aod::TrackSelection, aod::StoredTracksIU, aod::TracksCovIU, aod::McTrackLabels>;
  using OriTracks = soa::Join<aod::Tracks, aod::TracksExtra, aod::TracksDCA, aod::TracksDCACov, aod::TrackSelection, aod::StoredTracksIU, aod::TracksCovIU, aod::JTracksTagExt, aod::McTrackLabels>;
  using OriTracksIU = soa::Join<aod::Tracks, aod::TracksExtra, aod::TracksDCA, aod::TracksDCACov, aod::TrackSelection, aod::StoredTracksIU, aod::TracksCovIU, aod::JTracksIUTagExt, aod::McTrackLabels>;
  ...
  void fillHistogramIPsMCD(T const& collision, U const& mcdjets, V const& tracks)
  {
    for (auto& mcdjet : mcdjets) {
      ...
      // common var
      std::vector<float> incjetTracksImpXY, lfjetTracksImpXY, cjetTracksImpXY, bjetTracksImpXY;
      std::vector<float> incjetTracksSignImpXY, lfjetTracksSignImpXY, cjetTracksSignImpXY, bjetTracksSignImpXY;
      std::vector<float> incjetTracksImpXYSig, lfjetTracksImpXYSig, cjetTracksImpXYSig, bjetTracksImpXYSig;
      std::vector<float> incjetTracksSignImpXYSig, lfjetTracksSignImpXYSig, cjetTracksSignImpXYSig, bjetTracksSignImpXYSig;
      std::vector<float> incjetTracksImpXYZ, lfjetTracksImpXYZ, cjetTracksImpXYZ, bjetTracksImpXYZ;
      std::vector<float> incjetTracksSignImpXYZ, lfjetTracksSignImpXYZ, cjetTracksSignImpXYZ, bjetTracksSignImpXYZ;
      std::vector<float> incjetTracksImpXYZSig, lfjetTracksImpXYZSig, cjetTracksImpXYZSig, bjetTracksImpXYZSig;
      std::vector<float> incjetTracksSignImpXYZSig, lfjetTracksSignImpXYZSig, cjetTracksSignImpXYZSig, bjetTracksSignImpXYZSig;
      ...
      //for (auto& track : tracks) {
        ...
        //auto trackiu = track;
      for (auto& jettrack : mcdjet.template tracks_as<JetTagTracks>()) {
        auto track = jettrack.template track_as<OriTracks>();
        auto trackiu = jettrack.template track_as<OriTracksIU>();
        auto deltaR = JetUtilities::deltaR(mcdjet, track);
        if (deltaR > mcdjet.r() / 100.0) {
          //LOGF(info, Form("track outside jet", deltaR));
          continue;
        }
        int sign; 
        float varImpXY, varSgnImpXY, varImpXYSig, varSgnImpXYSig, varImpXYZ, varSgnImpXYZ, varImpXYZSig, varSgnImpXYZSig;
        ...
        /// TEMP ///
        registry.fill(HIST("h_inc_jet_distance_x_track_pv_global"), JetTaggingUtilities::getdcaXGlo(collision, mcdjet, track));
        registry.fill(HIST("h_inc_jet_distance_x_track_pv_local"), track.x()-collision.posX());
        registry.fill(HIST("h_inc_jet_dca_x_track_pv"), track.dcaX());
        registry.fill(HIST("h_inc_jet_dca_y_track_pv"), track.dcaY());
        registry.fill(HIST("h_inc_jet_abs_dca_xy_track_pv"), std::sqrt(track.dcaX()*track.dcaX()+track.dcaY()*track.dcaY()));
        registry.fill(HIST("h_inc_jet_abs_impact_parameter_xy"), std::abs(varImpXY));
        /// compare between track and track iu for dca
        registry.fill(HIST("h_inc_jet_distance_x_trackiu_pv_global"), JetTaggingUtilities::getdcaXGlo(collision, mcdjet, trackiu));
        registry.fill(HIST("h_inc_jet_dca_x_trackiu_pv"), trackiu.dcaX());
        ...
        if (jetflavor == JetTaggingSpecies::lightflavour) {
          ...
          lfjetTracksSignImpXYZ.push_back(varSgnImpXYZ);
          lfjetTracksImpXYZSig.push_back(varImpXYZSig);
          lfjetTracksSignImpXYZSig.push_back(varSgnImpXYZSig);
        }
        if (jetflavor == JetTaggingSpecies::charm) {
          ...
          cjetTracksSignImpXYZ.push_back(varSgnImpXYZ);
          cjetTracksImpXYZSig.push_back(varImpXYZSig);
          cjetTracksSignImpXYZSig.push_back(varSgnImpXYZSig);
        }
        if (jetflavor == JetTaggingSpecies::beauty) {
      ...
      // Track counting
//      std::cout << "----------------------------------" << "\n";
//      std::cout << "BEFORE Elements of the vector: " << "\n";
//      for (int i = 0; i < incjetTracksSignImpXY.size(); ++i) {
//        //std::cout << incjetTracksImpXY[i] << " ";
//        std::cout << incjetTracksSignImpXY[i] << " ";
//      }
//      std::cout <<"\n";
      sort(incjetTracksImpXY.begin(), incjetTracksImpXY.end(), std::greater<float>());
      sort(incjetTracksImpXYSig.begin(), incjetTracksImpXYSig.end(), std::greater<float>());
      sort(incjetTracksSignImpXY.begin(), incjetTracksSignImpXY.end(), std::greater<float>());
      sort(incjetTracksSignImpXYSig.begin(), incjetTracksSignImpXYSig.end(), std::greater<float>());
      sort(incjetTracksImpXYZ.begin(), incjetTracksImpXYZ.end(), std::greater<float>());
      sort(incjetTracksImpXYZSig.begin(), incjetTracksImpXYZSig.end(), std::greater<float>());
      sort(incjetTracksSignImpXYZ.begin(), incjetTracksSignImpXYZ.end(), std::greater<float>());
      sort(incjetTracksSignImpXYZSig.begin(), incjetTracksSignImpXYZSig.end(), std::greater<float>());

//      std::cout << "AFTER  Elements of the vector: " << "\n";
//      for (int i = 0; i < incjetTracksSignImpXY.size(); ++i) {
//        //std::cout << incjetTracksImpXY[i] << " ";
//        std::cout << incjetTracksSignImpXY[i] << " ";
//      }     
//      std::cout <<"\n";
      ...
      if (incjetTracksImpXY.size() > 0) { // N1
        ...
        registry.fill(HIST("h_inc_jet_sign_impact_parameter_xyz_N1"), incjetTracksSignImpXYZ[0]);
        registry.fill(HIST("h_inc_jet_impact_parameter_xyz_significance_N1"), incjetTracksImpXYZSig[0]);
        registry.fill(HIST("h_inc_jet_sign_impact_parameter_xyz_significance_N1"), incjetTracksSignImpXYZSig[0]);
      }
      ...
      if (incjetTracksImpXY.size() > 1) { // N2
        ...
        registry.fill(HIST("h_inc_jet_impact_parameter_xyz_N2"), incjetTracksImpXYZ[1]);
        registry.fill(HIST("h_inc_jet_sign_impact_parameter_xyz_significance_N2"), incjetTracksSignImpXYZSig[1]);
        registry.fill(HIST("h_inc_jet_sign_impact_parameter_xyz_significance_N2"), incjetTracksSignImpXYZSig[1]);
      }
      if (incjetTracksImpXY.size() > 2) { // N3
        ...
        registry.fill(HIST("h_inc_jet_sign_impact_parameter_xyz_N3"), incjetTracksSignImpXYZ[2]);
        registry.fill(HIST("h_inc_jet_sign_impact_parameter_xyz_significance_N3"), incjetTracksSignImpXYZSig[2]);
      }

      if (jetflavor == JetTaggingSpecies::lightflavour && !(lfjetTracksImpXY.empty())) { // lfjet
        sort(lfjetTracksImpXY.begin(), lfjetTracksImpXY.end(), std::greater<float>());
        sort(lfjetTracksImpXYSig.begin(), lfjetTracksImpXYSig.end(), std::greater<float>());
        sort(lfjetTracksSignImpXY.begin(), lfjetTracksSignImpXY.end(), std::greater<float>());
        sort(lfjetTracksSignImpXYSig.begin(), lfjetTracksSignImpXYSig.end(), std::greater<float>());
        sort(lfjetTracksImpXYZ.begin(), lfjetTracksImpXYZ.end(), std::greater<float>());
        sort(lfjetTracksImpXYZSig.begin(), lfjetTracksImpXYZSig.end(), std::greater<float>());
        sort(lfjetTracksSignImpXYZ.begin(), lfjetTracksSignImpXYZ.end(), std::greater<float>());
        sort(lfjetTracksSignImpXYZSig.begin(), lfjetTracksSignImpXYZSig.end(), std::greater<float>());
        if (lfjetTracksImpXY.size() > 0) { // N1
          ...
          registry.fill(HIST("h_lfjet_sign_impact_parameter_xyz_N1"), lfjetTracksSignImpXYZ[0]);
          registry.fill(HIST("h_lfjet_sign_impact_parameter_xyz_significance_N1"), lfjetTracksSignImpXYZSig[0]);
        }
        if (lfjetTracksImpXY.size() > 1) { // N2
          ...
          registry.fill(HIST("h_lfjet_sign_impact_parameter_xyz_N2"), lfjetTracksSignImpXYZ[1]);
          registry.fill(HIST("h_lfjet_sign_impact_parameter_xyz_significance_N2"), lfjetTracksSignImpXYZSig[1]);
        }
        if (lfjetTracksImpXY.size() > 2) { // N3
          ...
          registry.fill(HIST("h_lfjet_sign_impact_parameter_xyz_N3"), lfjetTracksSignImpXYZ[2]);
          registry.fill(HIST("h_lfjet_sign_impact_parameter_xyz_significance_N3"), lfjetTracksSignImpXYZSig[2]);
        }
      }
      if (jetflavor == JetTaggingSpecies::charm && !(cjetTracksImpXY.empty())) { // cjet
        sort(cjetTracksImpXY.begin(), cjetTracksImpXY.end(), std::greater<float>());
        sort(cjetTracksImpXYSig.begin(), cjetTracksImpXYSig.end(), std::greater<float>());
        sort(cjetTracksSignImpXY.begin(), cjetTracksSignImpXY.end(), std::greater<float>());
        sort(cjetTracksSignImpXYSig.begin(), cjetTracksSignImpXYSig.end(), std::greater<float>());
        sort(cjetTracksImpXYZ.begin(), cjetTracksImpXYZ.end(), std::greater<float>());
        sort(cjetTracksImpXYZSig.begin(), cjetTracksImpXYZSig.end(), std::greater<float>());
        sort(cjetTracksSignImpXYZ.begin(), cjetTracksSignImpXYZ.end(), std::greater<float>());
        sort(cjetTracksSignImpXYZSig.begin(), cjetTracksSignImpXYZSig.end(), std::greater<float>());
        if (cjetTracksImpXY.size() > 0) { // N1
          ...
          registry.fill(HIST("h_cjet_sign_impact_parameter_xyz_N1"), cjetTracksSignImpXYZ[0]);
          registry.fill(HIST("h_cjet_sign_impact_parameter_xyz_significance_N1"), cjetTracksSignImpXYZSig[0]);
        }
        if (cjetTracksImpXY.size() > 1) { // N2
          ...
          registry.fill(HIST("h_cjet_sign_impact_parameter_xyz_N2"), cjetTracksSignImpXYZ[1]);
          registry.fill(HIST("h_cjet_sign_impact_parameter_xyz_significance_N2"), cjetTracksSignImpXYZSig[1]);
        }
        if (cjetTracksImpXY.size() > 2) { // N3
          ...
          registry.fill(HIST("h_cjet_sign_impact_parameter_xyz_N3"), cjetTracksSignImpXYZ[2]);
          registry.fill(HIST("h_cjet_sign_impact_parameter_xyz_significance_N3"), cjetTracksSignImpXYZSig[2]);
        }
      }
      if (jetflavor == JetTaggingSpecies::beauty && !(bjetTracksImpXY.empty())) { // bjet
        sort(bjetTracksImpXY.begin(), bjetTracksImpXY.end(), std::greater<float>());
        sort(bjetTracksImpXYSig.begin(), bjetTracksImpXYSig.end(), std::greater<float>());
        sort(bjetTracksSignImpXY.begin(), bjetTracksSignImpXY.end(), std::greater<float>());
        sort(bjetTracksSignImpXYSig.begin(), bjetTracksSignImpXYSig.end(), std::greater<float>());
        sort(bjetTracksImpXYZ.begin(), bjetTracksImpXYZ.end(), std::greater<float>());
        sort(bjetTracksImpXYZSig.begin(), bjetTracksImpXYZSig.end(), std::greater<float>());
        sort(bjetTracksSignImpXYZ.begin(), bjetTracksSignImpXYZ.end(), std::greater<float>());
        sort(bjetTracksSignImpXYZSig.begin(), bjetTracksSignImpXYZSig.end(), std::greater<float>());
        if (bjetTracksImpXY.size() > 0) { // N1
          ...
          registry.fill(HIST("h_bjet_sign_impact_parameter_xyz_N1"), bjetTracksSignImpXYZ[0]);
          registry.fill(HIST("h_bjet_sign_impact_parameter_xyz_significance_N1"), bjetTracksSignImpXYZSig[0]);
        }
        if (bjetTracksImpXY.size() > 1) { // N2
          ...
          registry.fill(HIST("h_bjet_sign_impact_parameter_xyz_N2"), bjetTracksSignImpXYZ[1]);
          registry.fill(HIST("h_bjet_sign_impact_parameter_xyz_significance_N2"), bjetTracksSignImpXYZSig[1]);
        }
        if (bjetTracksImpXY.size() > 2) { // N3
          ...
          registry.fill(HIST("h_bjet_sign_impact_parameter_xyz_N3"), bjetTracksSignImpXYZ[2]);
          registry.fill(HIST("h_bjet_sign_impact_parameter_xyz_significance_N3"), bjetTracksSignImpXYZSig[2]);
        }
      }
    }
  }
  ...
  //void processDummy(aod::Collision const& collision, aod::StoredTracksIU const& tracks)
  ...
  //void processIPsMCD(aod::Collision const& collision, JetTagTable const& mcdjets, JetTagTracks const& tracks, OriTracks const& oritracks, aod::JMcParticles const& particles)
  //void processIPsMCD(soa::Join<aod::JCollisions, aod::JCollisionPIs>::iterator const& jcollision, JetTagTable const& mcdjets, JetTagTracks const& tracks, OriTracks const& oritracks, aod::JMcParticles const& particles)
  void processIPsMCD(soa::Join<aod::JCollisions, aod::JCollisionPIs> const& jcollisions, aod::Collisions const& collisions, JetTagTable const& mcdjets, JetTagTracks const& tracks, OriTracks const& oritracks, OriTracksIU const& oritracksiu, aod::JMcParticles const& particles)
  {
    for (auto &jcollision : jcollisions) {
      auto oricoll = jcollision.template collision_as<aod::Collisions>();
      fillHistogramIPsMCD(oricoll, mcdjets, oritracks);
    }
  }
...
//using JetTaggerTCChargedMCDJets = JetTaggerHFTC<soa::Join<aod::ChargedMCDetectorLevelJets, aod::ChargedMCDetectorLevelJetConstituents>>;
